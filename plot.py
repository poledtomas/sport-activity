...
if __name__ == "__main__":
    ...
    actual_2025 = df_daily[df_daily['Čas'].dt.year == 2025]['Stoupání (m)'].sum()
    days_elapsed = (today - datetime(2025, 1, 1)).days + 1
    days_remaining = (end_of_year - today).days
    avg_daily = actual_2025 / days_elapsed

    scenarios = {
        'Skutečnost (do 15.7.)': actual_2025,
        'Lineární tempo': actual_2025 + avg_daily * days_remaining,
        'ML predikce': actual_2025 + future_df['predikce (m)'].sum(),
        '+20 % intenzita': actual_2025 + (avg_daily * 1.2 * days_remaining),
        '–20 % intenzita': actual_2025 + (avg_daily * 0.8 * days_remaining),
    }
